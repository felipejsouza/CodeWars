...
def find_even_index(arr):
    ...
    inspecting = [[np.sum(numbers[(i+1):]) , np.sum(numbers[:i])] for i in range(len(numbers))]
    partial_results =[ np.array_equal(pair[0],pair[1]) for pair in inspecting]
    for word in partial_results:
      if word == True:
        index = i
        right_sum = inspecting[i][0]
        return index
        break
      i+=1
    if (i == len(numbers)): return -1
